...
C-----------------------------------------------------------
      ...
      WEIN=WEI
C         IF(I3C*J.NE.0) THEN
C      FF12=DCMPLX(1.D0,0.D0)
C      FF21=DCMPLX(1.D0,0.D0)
C      JRAT=1
C      CALL VZ(WEIN) ! weight due to particle-particle FSI
C         ENDIF
         RETURN
      END
C=======================================================================
...
C--   calculating the derivatives C(5)= dC(3)/dk^2, C(6)= dC(4)/dk^2
	...
	ENDIF
     ...
     ,        AAPAPI(3,ISPIN)*AK2S0/2)
      ...
      RETURN
      END
          SUBROUTINE NSINI(NSINPUT,AACUT)
        COMMON/FSI_NS/LL,NS,ICH,ISI,IQS,I3C,I3S
        NS=NSINPUT
        IF(NS.EQ.1)AA=0.D0        
        IF(NS.EQ.4)AA=AACUT
...
C---  charge of the first and second particle
     ...
     C         -1.D0,-1.D0,28*.0D0/
C---MSPIN vs (LL)
      DATA MSPINH/2,2,2,4*1,2*2,8*1,3,1,2,1,2*1,2*2,1,3*2, 2,
     C            2*1,28*.0D0/ !qiao
C---Spin factors RHO vs (LL,ISPIN)
     ...
     3          2*0.D0,28*0.D0, 420*0.D0/
        RHOH(2,1)=0.25
        RHOH(2,2)=0.75
C      print *,"RHO,RHO: ",RHOH(2,1),RHOH(2,2)!qiao
...
cc     2 -10.8D0,2*-5.4D0,4*0.D0,-6.35D0,-11.88D0,8*0.D0,9*0.D0,
     ...
     2  2*0.D0,28*0.D0,
     3  480*0.D0/
C        print *,"FD1,FD2: ",FDH(2,1),FDH(2,2)!qiao
c--------|---------|---------|---------|---------|---------|---------|----------
...
C     1  10.6D0, ! NSC97e potential /\/\
     ...
     2  2*0.D0,28*0.D0,
     3  480*0.D0/
C        print *,"RD1,RD2: ",RDH(2,1),RDH(2,2)!qiao
...
ccc      IF(NS.EQ.2.OR.NS.EQ.4)AA=.1D0 !!in 1/GeV --> .02 fm
      IF(NS.EQ.2.OR.NS.EQ.4)AA=5D0 !!in 1/GeV --> 1.0 fm       //qiao
...
C-- divided by Coulomb penetration factor squared (if ICH=1)
      IF(NS.NE.1)GOTO 230
C      IF(LL.NE.4)GOTO 230 ! SW scat. amplitude used for alfa-alfa only
C      !qiao
      GAK=G(AK)
      AKACH=AK
