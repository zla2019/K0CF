...
double twogaus(Double_t *x,Double_t *par) // Gaus function
{
  Double_t fitsig = gaus(x, par) + gaus(x, &par[3]) ;
  return fitsig;
}
...
double doublegaus(Double_t *x,Double_t *par)
{
	...
	Double_t fitsig = (par[0]/(par[2]*sqrt(2*3.14159265358979323846)))*TMath::Exp(-0.5* ((x[0]-par[1])/par[2])* ((x[0]-par[1])/par[2]) ) + (par[3]/(par[4]*sqrt(2*3.14159265358979323846)))*TMath::Exp(-0.5* ((x[0]-par[1])/par[4])* ((x[0]-par[1])/par[4]) );
        return fitsig;
}

double fstOrderPol(Double_t* x, Double_t* par)
{
	Double_t fitbg = par[0]+par[1]*x[0];
	return fitbg;
}

double FstorderpolDoublegaus(Double_t* x, Double_t* par)
{
	Double_t fitResult = doublegaus(x, &par[0]) + fstOrderPol(x, &par[5]);
	return fitResult;
}

double FstorderpolGaus(Double_t* x, Double_t* par)
{
	Double_t fitResult = gaus(x, &par[0]) + fstOrderPol(x, &par[3]);
	return fitResult;
}
...
double fitFuncDoubleGaus(Double_t *x,Double_t *par) //doublegaus + pol2
{
...
double fitFuncExp2(Double_t *x,Double_t *par) //mT exponential
{
  Double_t fitdNdy = par[0]*exp(-(sqrt(x[0]*x[0] + 0.89594*0.89594)-0.89594)/par[1])*x[0];
//  Double_t fitdNdy = par[0]*exp(-x[0]/par[1]);
  return fitdNdy * x[0];
...
double fitFuncExp3(Double_t *x,Double_t *par) //mT exponential
{
  Double_t fitdNdy = par[0]*exp(-(sqrt(x[0]*x[0] + 0.89594*0.89594)-0.89594)/par[1]);
//  Double_t fitdNdy = par[0]*exp(-x[0]/par[1]);
  return fitdNdy;
}
...
Double_t IntegrandBGBW(const double *x, const double *p)
{
  ...
  double beta = p[1];
  double T    = p[2];
  double mT   = Sqrt(ximass*ximass + pT*pT);
  double n    = p[3];
...
Double_t BGBWPt5(const double *x, const double *p)
{
  double radius = 1.;
  double Y = 1;
  double pi = TMath::Pi();
...
Double_t BGBWPt(const double *x, const double *p)
{
  ...
  double params[5] = {pT//(sqrt(pt2 + m02) - m0), beta, T, n, mass};
//  return fIntBG->Integral(0., radius, params) * p[3] * x[0];
  return fIntBG->Integral(0., radius) * p[3] * x[0];
}*/
...
Double_t BGBWMt(const double *x, const double *p)
{
  ...
  double Y = 1;
  double pi = TMath::Pi();
  double mT   = x[0] + ximass;
	double pT = sqrt(mT*mT - ximass*ximass);
  double mass = p[4];
  double beta = p[0];
  double T    = p[1];
  double n    = p[2];
...
double integral(double beta_T, double T, double n, double pt, double mt)
{
  ...
  double s = 0.;
  for(double r = dr/2; r < rc; r += dr)
  {
    double beta_r = (beta_T*(n+2)/2) * TMath::Power((r/rc),n);
...
double getAmplitudeOld(double T, double beta_T, double n, double mass, TGraphErrors *g){
  ...
  for( int pt = 0; pt < g->GetN(); pt++){
    vX.push_back(  g->GetX()[pt] );
    vY.push_back(  g->GetY()[pt] );
    veY.push_back( g->GetEY()[pt] );
  }
...
//	TF1* fIntBG2 = new TF1("fIntBG2", IntegrandBGBW, 0., 1., 5);
//	fIntBG2->SetParameters(pt, beta_T, T, n, m);
//	fIntBG2->FixParameter(0, pt);
//	fIntBG2->FixParameter(4, m);
//    fn[i] = mt * fIntBG2->Integral(0, 1.);
    fn[i] = mt * integral(beta_T, T, n, pt, mt);
  }
  ...
  {
    if(veY[i]>0){
      Sum1 += vY[i]*fn[i] / pow(veY[i],2);
      Sum2 += pow(fn[i] / veY[i], 2);
      //                Sum1 += y2[i];// * x[n][c][i];
      //                Sum2 += fn[i];// * x[n][c][i];
    }
  }
  double aka = (Sum2>0.) ? Sum1/Sum2 : 0.;
  return aka;
...
double MyFunc2( double *pt, double *p){
  ...
  double aka = getAmplitudeOld(p[1], p[0], p[2], mass, gpart[irap2][nMul]);
//	TF1* fIntBG = new TF1("fIntBG", IntegrandBGBW, 0., radius, 5);
//	fIntBG->SetParameters(pt[0], p[0], p[1], p[2], p[3]);
//	fIntBG->FixParameter(0, pt[0]);
//	fIntBG->FixParameter(4, p[3]);
//  temp = aka * pt[0] * fIntBG->Integral(0., radius);// * pt[0];
  temp = aka * mt * integral(p[0],p[1],p[2],pt[0],mt);
//  temp = aka * mt * integral(p[0],p[1],p[2],pt[0],mt) * pt[0];
//	delete fIntBG;

  return temp;
}

double MyFunc3( double *pt, double *p){
...
double MyFunc4( double *x, double *p){
  ...
  double temp = 0.;
  double aka = getAmplitudeOld(p[1], p[0], p[2], mass, gpart[irap2][nMul]);
  temp = aka * mt * integral(p[0],p[1],p[2],pt,mt);

  return temp;
}

Double_t BGBWPt3(const double *x, const double *p)
{
  double radius = 1.;
  double Y = 1;
  double pi = TMath::Pi();
  ...
  double params[5] = {pT, beta, T, n, mass};
	double scale = getAmplitudeOld(T, beta, n, mass, gpart[irap2][nMul]);
//  return fIntBG->Integral(0., radius, params) * p[3] * x[0];
  return fIntBG->Integral(0., radius) * x[0] * scale;
}

double ptmt(double mt){
 
 double m0 = 0.493677;
 double pt = sqrt((mt+m0)*(mt+m0)-m0*m0);
