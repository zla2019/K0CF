...
std::complex<double> amplitude(double tKstar)
{
	...
	return 0.5 * (f0(tKstar) + f1(tKstar));
}

std::complex<double> f0(double tKstar)	//f0(980)
{
	double s = 4 * (MassK0s*MassK0s + tKstar*tKstar * hbarc*hbarc);
	std::complex<double> a = std::complex<double>(0, GammaF0KK * tKstar);
	//std::complex<double> b = std::complex<double>(0, 0.05 * 0.973);
	std::complex<double> b = std::complex<double>(0, GammaF0PiPi * f0refMom(tKstar) / hbarc);
...
std::complex<double> f1(double tKstar)	//a0(980)
{
	...
	std::complex<double> b = std::complex<double>(0, GammaA0PiPi * a0refMom(tKstar) / hbarc);
...
{
	return sqrt(tKstar*tKstar * hbarc*hbarc + MassK0s*MassK0s - MassPi*MassPi);
}
